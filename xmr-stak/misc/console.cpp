...
void win_exit(int code)
{
	...
	if(envSize == 0)
	{
	printer::inst()->print_str("Press any key except the Windows key, Context key, SHIFT, NUMLOCK, CAPSLOCK, ScrollLock, CTRL, ALT, neither F1-F12, nor Sleep key to exit. :}");
		get_key();
	}
	std::exit(code);
}
